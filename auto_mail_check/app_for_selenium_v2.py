...
es.get('https://webmail.jikei.ac.jp/')
es.write_all([['input[name="twuser"]',id],['input[name="twpassword"]',password]])
es.click('input[type="submit"]')

es.click('input[value="クリア"]')
for i in range(4):
    es.click('div[style*="/images/imatrix/a1.gif"]')
es.click('input[value="ログイン"]')


########## 操作関数定義 ##########
def check_mail_and_notify():
    #try:
    es.ec_wait_text('#contentIframe h3','新着メール情報')
    es.click('table#rmail')
    es.iframe_in('#contentIframe')
